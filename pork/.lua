...
local function GetConfigFolder(configName)
    return "ProdigyConfigs/" .. configName
end

local function EnsureConfigFolder()
    if isfolder and not isfolder("ProdigyConfigs") then
        makefolder("ProdigyConfigs")
    end
end
...
local function GetAvailableConfigs()
    ...
    if isfolder and listfiles then
        EnsureConfigFolder()
        local files = listfiles("ProdigyConfigs")
        for _, file in ipairs(files) do
            local name = file:match("ProdigyConfigs/(.+)%.json$") or file:match("ProdigyConfigs\\(.+)%.json$")
            if name then
...
local ProdigyBlur = {}
ProdigyBlur.__index = ProdigyBlur

function ProdigyBlur.new(object)
...
function ProdigyBlur:_CreateDepthOfField()
    local existingDOF = lg:FindFirstChild("ProdigyBlur")
    if existingDOF then
        existingDOF:Destroy()
    end
    local existingBlur = lg:FindFirstChild("ProdigyBlurEffect")
...
function ProdigyBlur:_CreateFolder()
    local existingFolder = workspace.CurrentCamera:FindFirstChild("ProdigyBlur")
...
function ProdigyBlur:_CreateRoot()
...
function ProdigyBlur:_CreateFrame()
...
function ProdigyBlur:_Render(distance)
...
function ProdigyBlur:_Initialize()
...
function ProdigyBlur:SetEnabled(enabled)
...
function ProdigyBlur:Destroy()
    if self._folder then
        self._folder:Destroy()
    end
    local dof = lg:FindFirstChild("ProdigyBlur")
    if dof then
        dof:Destroy()
    end
    local blur = lg:FindFirstChild("ProdigyBlurEffect")
    if blur then
        blur:Destroy()
    end
end
...
function Library.new(title, configFolder)
    ...
    self.title = title or "Prodigy"
    self.configFolder = configFolder or title or "Prodigy"
    self.sections = {}
    self.currentTab = nil
    self.minimized = false
    self._ProdigyBlur = nil
    self._keybinds = {}
...
function Library:Toggle()
    ...
    if self._ProdigyBlur then
        self._ProdigyBlur:SetEnabled(self._visible)
    end
...
function Library:_CreateMainv0rtexd()
    ...
    self._ProdigyBlur = ProdigyBlur.new(self.container)
end
...
function Library:_ToggleMinimize()
    ...
    if self.minimized then
        ...
        CreateTween(self.container, {Size = UDim2.new(0, self.container.AbsoluteSize.X, 0, 55)}, animationspeed.Slow)
        if self.resizeBtn then
            self.resizeBtn.Visible = false
        end
    else
        if self._ProdigyBlur then
            self._ProdigyBlur:SetEnabled(true)
        end
...
function Library:SaveConfig(configName)
    ...
    local success, err = pcall(function()
        writefile("ProdigyConfigs/" .. configName .. ".json", hs:JSONEncode(configData))
    end)
...
function Library:LoadConfig(configName)
    ...
    local path = "ProdigyConfigs/" .. configName .. ".json"
